...
NIO_USER_ID = os.environ.get("NIO_USER_ID")
NIO_PASSWORD = os.environ.get("NIO_PASSWORD")
...
class CustomEncryptedClient(AsyncClient):
    ...
    async def login(self) -> None:
        ...
        if not self.user_id or not self.access_token or not self.device_id:
            ...
            resp = await super().login(NIO_PASSWORD)
...
async def run_client(client: CustomEncryptedClient) -> None:
    ...
    async def after_first_sync():
        ...
        # global variable
        client.trust_devices(NIO_ADMINS[0], NIO_DEVICE_IDS)
        ...
        client.trust_devices(NIO_USER_ID)
